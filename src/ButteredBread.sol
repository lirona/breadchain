 ...
 */
contract ButteredBread is ERC20VotesUpgradeable, OwnableUpgradeable {
    error NonTransferable();
    error InsufficientFunds();

    mapping(address butter => bool allow) public allowlistedLP;
    ...
    function initialize(address[] memory _liquidityPools) external initializer {
        ...
        for (uint256 i; i < _liquidityPools.length; ++i) {
            allowlistedLP[_liquidityPools[i]] = true;
        }
    }
